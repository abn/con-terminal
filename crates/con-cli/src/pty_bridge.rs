...
fn default_shell_args(program: &str) -> Vec<String> {
    let name = std::path::Path::new(program)
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or(program);
    match name {
        "nu" => vec!["--interactive".to_string()],
        "xonsh" => vec!["-i".to_string()],
        "pwsh" => vec!["-NoLogo".to_string()],
        "fish" => vec!["--login".to_string(), "--interactive".to_string()],
        _ => vec!["-l".to_string()],
    }
}
...
pub fn run_pty_bridge(args: PtyBridgeArgs) -> Result<()> {
    ...
    let (program, default_args) = match args.program {
        Some(prog) => {
            let def = if args.args.is_empty() {
                default_shell_args(&prog)
            } else {
                vec![]
            };
            (prog, def)
    ...
    exit_frame[1..5].copy_from_slice(&exit_code.to_be_bytes());
